#!/usr/bin/env bash
# Daily release check for __PKG__.
#
...
# __PKG__.tar.gz asset directly — no git involved, so this works the same
...
# must never hold up a session. `curl` is invoked under `timeout` for exactly
